...
export default function SettingsPage() {
  const { assets, transactions, exportData, importData } = useStore()
      ...
      <main className="flex-1 p-6">
        <div className="mx-auto flex max-w-[1200px] flex-col gap-6">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">管理资产类型与应用配置</p>
          </div>
              ...
              {types.map((t, i) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="flex h-5 w-5 items-center justify-center rounded text-muted-foreground transition-colors hover:bg-rose-50 hover:text-rose-500"
                  >
          ...
          <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
            ...
            <div className="mt-4 flex gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-lg border border-border bg-card px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-secondary active:scale-95"
              >
                {importStatus === "success" ? <Check className="h-4 w-4 text-emerald-500" /> :
                 importStatus === "error" ? <AlertCircle className="h-4 w-4 text-rose-500" /> :
          ...
          <div className="rounded-xl border border-border bg-card p-5 shadow-sm">
            ...
            <div className="mt-4 flex flex-col gap-2 text-sm">
              <div className="flex justify-between py-1.5">
                ...
                <span className="font-medium text-foreground">{aboutInfo.name}</span>
              </div>
              <div className="flex justify-between border-t border-border py-1.5">
                <span className="text-muted-foreground">版本</span>
                <span className="font-medium text-foreground">{aboutInfo.version}</span>
              </div>
              <div className="flex justify-between border-t border-border py-1.5">
                <span className="text-muted-foreground">技术栈</span>
                <span className="font-medium text-foreground">{aboutInfo.tech}</span>
              </div>
              <div className="flex justify-between border-t border-border py-1.5">
                <span className="text-muted-foreground">许可证</span>
                <span className="font-medium text-foreground">{aboutInfo.license}</span>
              </div>
            </div>
          </div>
        </div>
